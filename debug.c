...
void check_sub(MQTT_Packet packet, int connfd) {
    printf("Handle... \n");
...
MQTT_Packet receive_packet_from_client(int connfd) {
    ...
    printf("Offset -> receive_packet_from_client (0): %zu\n", offset);
    ...
    printf("Offset -> receive_packet_from_client (1): %zu\n", offset);
    ...
    printf("Offset -> receive_packet_from_client (2): %zu\n", offset);

    // Calculate payload size
    size_t payload_size = bytes_received - offset;
    printf("Payload_sie -> receive_packet_from_client: %zu\n", payload_size);
...
void *process_connection(void *arg) { 
    ...
    printf("Payload: ");
    for (int i = 0; i < strlen(received_packet.payload); i++) {
        printf("%02X ", received_packet.payload[i]);
    }
    printf("\n");
    ...
    printf("Payload len: %ld\n", strlen(received_packet.payload));
